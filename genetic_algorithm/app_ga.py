...
dr = DimReduction(log_dir=path, dimensions=3, frequency=2, plot_runtime=True)
callback_list.append(dr)

ga.compile(config=config, fitness_function=rf, callbacks=callback_list)

print("Run genetic algorithm\n")

ga.run()

# TODO: own config file, flags, graphic_config, TSNE
